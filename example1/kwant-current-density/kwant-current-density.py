...
#===========================================================
    ...
    J_0 = kwant.operator.Current(syst,where=[(syst.sites[tmp3[0][i][0][0]],syst.sites[tmp3[0][i][0][1]]) for i in range(hopping_number)])

#    current = J_0(wf(0)[0])
    total_current = sum(J_0(p) for p in wf(0))

    outfile = open('current_density.dat','w')

    for i in range(hopping_number):
        outfile.write('{0:14.7f} {1:14.7f} {2:14.7f} {3:14.7f} {4:14.7f} {5:14.7f} {6:40.30f} \n' .format(tmp4[0][i][0],tmp4[0][i][1],tmp4[0][i][2],tmp4[1][i][0],tmp4[1][i][1],tmp4[1][i][2],total_current[i]))
   
    outfile.close() 
#    for i in range(hopping_number):
#        print(tmp4[0][i][0],tmp4[0][i][1],tmp4[0][i][2],tmp4[1][i][0],tmp4[1][i][1],tmp4[1][i][2],total_current[i])
#
...
kwant_build(
    ...
    general_current_density_energy = -0.25,
    )
