...
def kwant_build(device_hami,device_coord,electrode_hami,electrode_coord,threshold,general_lattice,general_device_del,general_device_lead,general_electrode_basis_num,general_density_energy):
    ...
    lat = kwant.lattice.Polyatomic(primitive_vectors, basis_sites, norbs=1)
    syst = kwant.Builder()
    basis = lat.sublattices
    for i in range(general_device_del,kwant_family_size-general_device_del):
        syst[basis[i](0,0,0)] = hami[i][i]
...
#    kwant.plot(syst,site_size=0.20, site_lw=0.01, hop_lw=0.01,site_color=family_colors)
#================================================================================================
#===========================================================
...
#        print(tmp1[0][i][0])
    ...
    total_density = sum(density(p) for p in wf(0))

    outfile = open('density.dat','w')

    for i in range(site_number):
        outfile.write('{0:14.7f} {1:14.7f} {2:14.7f} {3:40.30f} \n' .format(tmp2[i][0],tmp2[i][1],tmp2[i][2],total_density[i]))
   
    outfile.close() 


kwant_build(
    device_hami='../../CP2K-Kwant-PCBM-4-layer-Au-periodic-1.5A-noopt/TEST-zang-test-1_0_202.Log', 
    device_coord='../../CP2K-Kwant-PCBM-4-layer-Au-periodic-1.5A-noopt/input.xyz',
    electrode_hami='../../CP2K-Kwant-8-layer-5x5-Au-isolate/TEST-zang-test-1_0_52.Log',
    electrode_coord='../../CP2K-Kwant-8-layer-5x5-Au-isolate/gold-5x5x8.xyz',
    threshold = 0.00001,
    general_lattice = 8.1564,
